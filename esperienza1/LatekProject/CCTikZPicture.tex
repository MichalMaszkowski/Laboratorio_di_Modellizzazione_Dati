...
\begin{Figure}
    ...
    \begin{tikzpicture}
        ...
        % Velocities
        \draw[->] (0,0) -- (2,0) node[right, above] {$\bm{v}_{LSR}$};
        \draw[->] (0,0) -- (0.4,0) node[right, above] {$\bm{V}_{0}$};
        \draw[->] (0,0) -- (0,-0.4) node[below, left] {$\bm{U}_{0}$};

        \draw[->] (3, -3) -- (3.66, -5) node[right, below] {$\bf{v_s}$};

        % rotational velocity of the sun around itself
        \draw[->] (-0.5, -1) arc (-120:-150:1.18);
        \draw node at (-1, -1) {$w_{sun}$}; 

    \end{tikzpicture}
    \captionof{figure}{Frames of reference.}\label{fig:FrameOfReference}
\end{Figure}
%\begin{multicols}{2}
